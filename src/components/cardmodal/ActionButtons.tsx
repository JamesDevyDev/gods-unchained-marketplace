...
type ActionButtonsProps = {
    ...
    newWallet: string | null
    loggedWallet: string | null
    onListingSuccess?: () => void  // ⭐ NEW: Callback to refresh listings
}
...
export const ActionButtons = ({ card, listingsData, loggedWallet, onListingSuccess }: ActionButtonsProps) => {
    const [activeTab, setActiveTab] = useState<'buy' | 'sell'>('buy')
    const [quantity, setQuantity] = useState(1)
    const [isBuying, setIsBuying] = useState(false)
    const [purchasedTokenId, setPurchasedTokenId] = useState('')
    ...
    const handleListNow = async () => {
        ...
        try {
            ...
            if (successCount > 0) {
                ...
                // ⭐ NEW: Trigger refresh of listings in parent component
                if (onListingSuccess) {
